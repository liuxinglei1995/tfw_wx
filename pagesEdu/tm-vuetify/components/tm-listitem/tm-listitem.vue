	...
	<view class="tm-listitem ">
		<view
			...
			:class="[
				...
				`round-tl-${pz_themeCus.round[0]}`,
				`round-tr-${pz_themeCus.round[1]}`,
				`round-bl-${pz_themeCus.round[2]}`,
				`round-br-${pz_themeCus.round[3]}`,
				border?'border-a-1':''
			]"
		>
			<view :class="['px-' + pz_themeCus.padding[0], 'py-' + pz_themeCus.padding[1]]">
				<view class="flex-between ">
					<view class="left flex-start ">
						...
						<view
						class="tm-content"
						:class=" labelInline ? 'flex-start':'flex-col'"
						style="width: 90%;">
							<view
							class=" flex-shrink"
								:class="[disabled===true?' text-grey-darken-1 ':(black_tmeme ? 'text-white' : `text-${titleColor}`),labelInline ? 'mr-20':'fulled']"
								:style="{
					...
					<view class="right text-grey-lighten-1 flex-end" :class="[black_tmeme ? 'bk' : '']">
						<view class="text-size-s pr-10" :style="'color:'+valueColor">
 ...
 * @property {Number} round = [4,4,4,4] 默认：4，单位upx, 外部圆角
 ...
 * @property {String} value-color = [] 默认："primary"，右图标颜色。
 ...
 * @property {Boolean} label-inline = [true|false] 默认：false，说明文字是否同行显示
