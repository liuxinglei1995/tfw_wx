<template>
	<view class="htz-image-upload-list">
		<view class="htz-image-upload-Item" v-for="(item,index) in uploadLists" :key="index">
			...
			<view v-else style="width: 100%;height: 100%;position: absolute;">
				<image :src="getFileUrl(item)" @click="imgPreview(getFileUrl(item))"></image>
				<view v-if="ifSelect" class="tm-upload-tips text-size-xs round-b-2 text-align-center"
				 @click="imgTagsShow = true,clickIndex = index"
				 style="background-color: rgba(0,0,0,0.3) !important;color: #FFF !important;position: relative;top: -40rpx;"
		...
		>
			<view class="py-15 px-25" style="background-color: #F0f0f0;height: 370rpx;">
				...
				:color="tagItem.id == uploadLists[clickIndex].data.room_type?'primary':'white'"
				size="g" 
				@click="selectImgTags(clickIndex,tagItem.id,tagItem.title)"
				:shadow="0" 
