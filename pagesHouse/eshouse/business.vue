<template>
	<view class="home">
		<tm-menubars style="position: fixed;z-index: 999999;" :title="house_name+'租售中心'"
		...
		<view class="wrap">
				...
				<view v-if="searchData.length>0">
					<scroll-view style="height: 70vh;" scroll-y="true" class="scroll-Y">
						<view v-for="item in searchData" :key="item.id" class="li flex-start" :data-xq="item.xq"
							...
							<text v-if="query.type==3"
								style="font-size: 26rpx;color: #D50000;font-weight: bold;line-height: 62rpx;width:100rpx ;">
								{{item.shopsHouseNum}}套
							</text>
							<text v-if="query.type==2"
								style="font-size: 26rpx;color: #D50000;font-weight: bold;line-height: 62rpx;width:100rpx ;">
								{{item.officeHouseNum}}套
							</text>
						</view>
					</scroll-view>
				</view>
			...
			<view class="px-30 py-10" v-if="xq_info">
				...
				<view class="text-size-s pt-10">
					...
					<view class="pr-10" style="color: #b9b9b9;display: flex;">
						<view class="" v-if="query.type==3">
							商铺{{xq_info.shangpu_count}}套
						</view>
						<view class="" v-if="query.type==2">
							写字楼{{xq_info.xiezilou_count}}套
						</view>
					</view>
				</view>
			</view>

				<index-house v-if="fwlx==0 || fwlx==1" :homeFeuer="houselist" :houseid="2" :househead="2"></index-house>
				<index-house v-else :homeFeuer="houselist" :houseid="2" :househead="2"></index-house>
...
<script>
	...
	export default {
		...
		onShareAppMessage(res) {
			return {
				title: '自家店  实在', 
				path: '/pagesHouse/eshouse/business?type='+this.query.type+'&house_name='+this.house_name,
			}
		},
		methods: {
			gethousetitle(query) {
				gethouseTitle(query).then(response => {
					...
					console.log(this.houselist, "kkkkkkkllllll");
