			...
			<view class="sub-menu-class" :class="{'show':showPage==page_index,'hide':pageState[page_index]!=true}">
				...
				<block v-if="(page.type=='hierarchy'||page.type=='hierarchy-column')&& page.submenu.length>0">
					...
					<scroll-view class="sub-menu-list" :class="[activeMenuArr[page_index].length>1?'first':'alone']"
						...
						<view v-if="(page.type=='hierarchy' && page.showInput == 'true')" style="position: relative;">
							<input class="price-input" v-model="min" type="number" focus placeholder="最低价" />
							<text style="line-height: 56rpx;margin: 10rpx;padding: 4rpx;"> — </text>
							<input class="price-input" v-model="max" type="number" focus placeholder="最高价" />
							<view class="price-input-btn" @tap="confirm">
								确定
							</view>
						</view>
					</scroll-view>
					...
					<view class="filter">
						<scroll-view class="menu-box" :scroll-y="true">
							<view class="box" v-for="(box,box_index) in page.submenu" :key="box_index">
								<view class="title">{{box.name}}</view>
								<view class="labels">
									<view v-for="(label,label_index) in box.submenu" style="width: 30%;">
										<view v-if="label.type != null && label.type === 'inputs'" style="position: relative;">
											<view>{{label.name}}</view>
											<input class="price-input" v-model="minPrice" type="number" focus placeholder="最小面积" />
											<text style="line-height: 56rpx;margin: 10rpx;padding: 4rpx;"> — </text>
											<input class="price-input" v-model="maxPrice" type="number" focus placeholder="最大面积" />
										</view>
										<view v-else :key="label_index" :class="{'on':label.selected}" style="width: 100%;height: 100%;"
	...
	export default {
		...
		methods: {
			initMenu() {
				...
				for (let i = 0; i < this.menuData.length; i++) {
					...
					tmpMenu.push({
						...
						type: tmpitem.type,
						showInput: tmpitem.showInput == 'true' ? 'true' : 'false'
					});
					//初始化选中项数组-ui状态
			...
			selectHierarchyMenu(page_index, level1_index, level2_index, level3_index) {
				...
				if (this.menu[page_index].type == 'hierarchy' && level2_index == null) {
					this.secondScrollInto = null
					this.activeMenuArr[page_index][1] = 0
					if (this.menu[page_index].showInput == 'true') {
						this.min = null;
						this.max = null;
					}
				}
			...
			selectRadioLabel(page_index, box_index, label_index) {
				...
				console.log(box_index, 'box_index')
			...
			confirm() {
				...
				if(this.minPrice != null && this.maxPrice != null) {
					value[5] = [this.minPrice, this.maxPrice]
				}
				console.log(value, 'value')
				//对结果做一下处理
				index.forEach((item, i) => {
					if (this.subData[i].showInput == 'true' && (this.min > 0 || this.max > 0)) {
						let submenu = {
							name: this.min + '-' + this.max + '元',
							submenu: {},
							value: this.min + '-' + this.max
						};
						this.menu[i].name = this.min + '-' + this.max + '元';
						...
						if (typeof(item[0]) == 'object') {
							...
							item.forEach((s, j) => {
								if (s != null) {
									...
									item[j] = s;
									s.forEach((v, k) => {
										value[i][j][k] = (v == null || v >= this.subData[i]
											.submenu[j].submenu.length) ? null : this.subData[
											i].submenu[j].submenu[v].value;
