<template>
	<view>
		<tm-menubars title="新增客源" iconColor="white"></tm-menubars>
		<view class="container pb-20">
			<tm-form @submit="submit" ref="formData" method="post">
				<uni-section style="margin-bottom: 10px;" title="需求区域">
					<template v-slot:right>
						<uni-data-picker :localdata="citiesList" @change="onPickerChange"></uni-data-picker>
					</template>
				</uni-section>

				<tm-input name="remarks" title="片区备注" v-model="reqData.remarks"></tm-input>

				<tm-pickers :default-value="reqData.sourceName" @confirm="sourceSelect" :list="dict.type.tfw_wtly"
					rang-key="label" name="source">
					<tm-input v-model="reqData.sourceName" disabled required title="委托来源"
						placeholder="请选择委托来源"></tm-input>
				</tm-pickers>
				<tm-pickers :default-value="reqData.publicAndPrivateName" @confirm="publicAndPrivateSelect"
					:list="dict.type.tfw_khlx" rang-key="label" name="publicAndPrivate">
					<tm-input v-model="reqData.publicAndPrivateName" disabled required title="客户类型"
						placeholder="请选择客户类型"></tm-input>
				</tm-pickers>
				...
				<tm-pickers :default-value="reqData.purposeOfTheHouseName" @confirm="purposeOfTheHouseSelect"
					:list="dict.type.tfw_property_type" rang-key="label" name="purposeOfTheHouse">
				...
				<view name="phoneNumber" v-for="(item,index) in customer" :key="index">
					...
					<tm-input title="业主手机号" required placeholder="手机号" v-model="item.value" class="uni-input"
						cursor-spacing="15" :adjust-position="true"></tm-input>

					<tm-button v-if="index !== 0" @click="removeDomain(item)">删除{{index+1}}</tm-button>
				</view>
...
<script>
	...
	export default {
		dicts: ['tfw_property_type', 'tfw_fuxing', 'sys_user_sex', 'tfw_wtly', 'tfw_khlx'],
		components: {
			UCheckbox,
			UCheckboxGroup,
			tmSliders,
		...
		onLoad: function(option) {
			if (option != null) {
				this.reqData.publicAndPrivateName = "私客"
				this.reqData.publicAndPrivate = 2
				this.customer.value = option.phonenumber;
			}
			this.citiesList = formatDataForUniPicker(provinceData, cityData, areaData)
		},
		...
		methods: {
			 ...
			 * @param {Object} e
			 */
			publicAndPrivateSelect(e) {
				this.reqData.publicAndPrivateName = e[0].data.label
				this.reqData.publicAndPrivate = e[0].data.raw.dictValue
			},
			onPickerChange(e) {
				console.log('Picker change:', e.detail.value);
				this.reqData.province = e.detail.value[0].value+"0000"
				this.reqData.city = e.detail.value[1].value+"00"
				this.reqData.zone = e.detail.value[2].value
			},
			 ...
			 */
			sourceSelect(e) {
				this.reqData.sourceName = e[0].data.label
				this.reqData.source = e[0].data.raw.dictValue
			},
			submit(e) {
				console.log(this.reqData)
				console.log(this.customer)
				if(this.reqData.province == null) {
					uni.showToast({
						title: '请选择需求区域!',
						icon: 'error',
						duration: 2000
				...
				if(this.reqData.sourceName == null) {
					uni.showToast({
						title: '请标注委托来源!',
						icon: 'error',
						duration: 2000
				...
				if(this.reqData.purposeOfTheHouseName == null) {
					uni.showToast({
						title: '请选择房屋用途!',
						icon: 'error',
						duration: 2000
