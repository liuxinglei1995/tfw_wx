				...
				<view @click.stop="onclick(index,item)" class="tm-grid-hover flex-center flex-col "  :style="{height:height_s+'px'}">
					...
					<view class=" pb-6 flex-shrink px-10 " style="">
						<view class="tm-grid-icon flex-shrink"  v-if="item.dot" >
							...
							<tm-badges style="z-index: 1;"  @click="clickDot(index,item)"  v-else-if="item.label" :offset="item.offset" :dot="false" :label="item.label" ></tm-badges>
							<tm-badges  @click="clickDot(index,item)" v-if="!item.dotIcon && !item.label" :offset="[10,0]" :dot="true"  ></tm-badges>
...
<script>
	...
	import tmBadges from "../tm-badges/tm-badges.vue"
