			...
			<view v-if="ifSelect">
				<view v-if="item.statusCode==3">
					<view v-if="tips&&!disabled" class="tm-upload-tips text-size-xs round-b-2"
					 ...
					 @click="imgTagsShow = true"
					 style="background-color: rgba(0,0,0,0.3) !important;color: #FFF !important;"
					 >{{item.data.room_type_cn}}</view>
					 <tm-poup
						:round="3"
						v-model="imgTagsShow"
						position="center"
						...
						:height="400"
					>
						<view class="py-15 px-25" style="background-color: #F0f0f0;height: 370rpx;">
							<tm-tags v-for="(tagItem,tagIndex) in imgTags" :key="tagIndex"
							:color="tagItem.id == item.data.room_type?'primary':'white'"
							size="g"
							@click="selectImgTags(index,tagItem.id,tagItem.title)"
							:shadow="0"
							model="fill">{{tagItem.title}}</tm-tags>
						</view>
					 </tm-poup>
				</view>
				<view v-else>
					<view v-if="tips&&!disabled" class="tm-upload-tips text-size-xs round-b-2"
					 :class="[
						 ...
						 item.statusCode==3?color_tmeme+' tex t':'',
						 black_tmeme?'bk':''
					 ]"
					 >{{item.status}}</view>
				</view>
...
<script>
	/**
	 * 上传图片组件
	 * @property {Boolean} ifSelect = [true|false] 默认false，是否选择图片标签
	 * @property {Array} imgTags = [] 默认：[],图片标签列表
	 * @property {Function} selectImgTags 选择图片标签触发
	...
	import tmTags from "../tm-tags/tm-tags.vue"
	export default {
		components:{tmIcons,tmPoup,tmTags},
		name: "tm-upload",
		props: {
			ifSelect:{
				type:Boolean,
		...
		methods: {
			 ...
			 * @param {Object} title
			 */
			selectImgTags(index,id,title){
				this.imgTagsShow = false
				this.$emit("selectImgTags",index,id,title)
			},
			//动态添加默认已上传的文件。
			pushFile(list){
				let t= this;
