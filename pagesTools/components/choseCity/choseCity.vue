		...
		<scroll-view class="scroll-view" scroll-y scroll-with-animation="true" enable-back-to-top="true"
			...
			<view class="block">
				...
				<view class="area list-item" id="area">
					...
					<view class="ul">
						<view class="li now font-clamp" @click="selectCity(myCityObj,'refresh')">
							<image
								src="https://fulu-mall.oss-cn-hangzhou.aliyuncs.com/e0e5e497b1714e8fa6135df32e4bf114.png"
								class="icon" v-if="hasLocation"></image>
							<image
								src="https://fulu-mall.oss-cn-hangzhou.aliyuncs.com/8ed89f5bcaac48c6bd9139261f86c6b6.png"
								class="icon" v-else></image>
				...
				<view class="area list-item" id="record" v-if="recordList.length">
					...
					<view class="ul">
						<view class="li font-clamp" v-for="item in recordList" :key="item.citycode"
...
<script>
	...
	let qqmapsdk = new QQMapWS.default({
		key: 'X4KBZ-W4Q6C-AEB2F-AJH3O-PK7ZF-FNFDH'
	});
	import {
		getcitylist
	} from '@/api/houst.js'
	...
	export default {
		...
		mounted() {
			let that = this;
			getcitylist().then(cityJson => {
				this.cityArr = cityJson.data.data.list
				...
				this.recordList = uni.getStorageSync('areaChooseHis')
				this.alphabet = cityJson.data.data.alphabet
				this.getLocation()
			})
		},
		watch: {
					...
					success: function(res) {
						...
						uni.request({
						    ...
						    success: (res) => {
								...
								var city = city.substring(0, city.indexOf('市'));
								var adData = {
									'name': city,
									'citycode': res.data.result.addressComponent.city_code.slice(-6)
								}
								that.setData({
									'myCityObj': adData
								})
								that.hasLocation = true
								uni.setStorageSync('nowCityObj', adData)
						    },
						});
						...
						// 		var adData = {
						// 			'name': city,
						// 			'citycode': (res.result.ad_info.city_code).replace(res
