	...
	export default {
		...
		methods: {
			...
			rateBindPickerChange(e) {
				...
				if(e.detail.value == 1) {
					this.form.baseRate = 0.03325
					this.form.rateNum = (0.03325 * 100).toFixed(3)
					this.rateIndex = e.detail.value;
				} else {
					this.form.baseRate = 0.0285 * this.rateList[e.detail.value].key
					this.form.rateNum = (0.0285 * this.rateList[e.detail.value].key * 100).toFixed(3)
					this.rateIndex = e.detail.value;
				}
			}
