<template>
	<view>
		...
		<view class="sear-select">
			<view class="left">
				<tm-pickers class="adr" @confirm="select" :default-value="selected"
					:list="['新房','二手房','商铺','写字楼','租房']">
					{{selected[0]}}
				...
				<view class="text">
					...
					<input type="text" :value="inputVal" @input="searchChange" confirm-type="搜索" @confirm="goSearch"
						placeholder="你想住在哪?" />
				</view>
			</view>
		</view>
		...
		<view class="searchHis">
			<tm-button @tap="selectSear(item)" v-if="history" v-for="(item,index) in history">{{item}}</tm-button>
		</view>
	</view>
...
<script>
	import tmPickers from '@/pagesTools/tm-vuetify/components/tm-pickers/tm-pickers.vue';
		...
		methods: {
			...
			goSearch: function() {
				...
				if (that.inputVal == '') {
					...
					switch (selected[0]) {
						case '新房':
							url = '/pagesHouse/newhouse/newhouse?type=1&input_val=' + searchVal
							break;
						case '商铺':
							url = '/pagesHouse/eshouse/eshouse?type=3&input_val=' + searchVal
							break;
						case '写字楼':
							url = '/pagesHouse/eshouse/eshouse?type=2&input_val=' + searchVal
							break;
						case '租房':
							url = '/pagesHouse/rentHouse/rentHouse?type=4&input_val=' + searchVal
							break;
						default:
							url = '/pagesHouse/eshouse/eshouse?type=0&input_val=' + searchVal
							break;
					}
					var history = uni.getStorageSync('houseSearch')
					var historyAll = [];
